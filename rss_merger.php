...
$script_version = '1.0';					/* Used for internal purposes */
...
function outputXML($rss_items) {
    global $site_name, $site_url, $xml_encoding, $num_of_items, $script_version;
	$output = '<?xml version="1.0" encoding="' . $xml_encoding . '"?>' . "\n";
	$output .= '<rss version="2.0">' . "\n";
	$output .= "\t" . '<channel>' . "\n";
	$output .= "\t\t" . '<title>' . $site_name . '</title>' . "\n";
	$output .= "\t\t" . '<link>' . $site_url . '</link>' . "\n";
	$output .= "\t\t" . '<description>A selection of my RSS collection</description>' . "\n";
	$output .= "\t\t" . '<pubDate>' . date(DATE_RFC822) . '</pubDate>' . "\n";
	$output .= "\t\t" . '<generator>http://www.makesites.cc/programming/by-makis/rss-merger-v' . $script_version . '/</generator>' . "\n";
